...
def main():
    ...
    f = lambda x: [int(i) for i in re.search(r'\d*\.\.\d*', x).group().split('..')]

    qseqids, _, stitles, pidents, sstarts, sends, lengths = list(zip(*results))

    pidents = list(map(float, pidents))
    sstarts = list(map(int, sstarts))
    sends = list(map(int, sends))
    lengths = list(map(int, lengths))
    qstarts, qends = list(zip(*(list(map(f, qseqids)))))
